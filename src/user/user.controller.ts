...
export class UserController {
  ...
  async getLoggedInUserInfo(@Req() request: Request) {
    const userInfo = await this.userService.getUserByAccountName(
      request['user'].taiKhoan,
    );
    return {
      content: userInfo,
    };
  ...
  async getMyBookedMovies(@Req() request: Request) {
    const userInfo = await this.userService.getBookedMoviesByUserId(
      request['user'].id,
    );
    return {
      content: userInfo,
